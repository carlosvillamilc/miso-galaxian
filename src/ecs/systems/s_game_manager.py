...
def system_game_manager(ecs_world:esper.World, c_game_manager:CGameManager, delta_time:float, current_level:int):
    if c_game_manager.state == GameState.START:
        ...
        if c_game_manager.current_game_time > c_game_manager.start_game_time:
            create_all_enemies(ecs_world, current_level)            
            ecs_world.delete_entity(c_game_manager.start_game_entity)
            c_game_manager.state = GameState.PLAY           